...
pubmed-proto owns *the shape of the `pubmed_proto` package*: which PubMed XML
elements survive into the schema, how they are named, and how they coerce. It
does **not** own the generation engine (that is `xsd-former`) nor how consumers
store or render the parsed articles.

## Language

**Generator** (this repo):
The inputs and build wiring that produce the package — `pubmed.dtd`,
`pubmed_transforms.yaml`, `Makefile`, `pyproject.toml`. Its own distribution is
`pubmed-proto-generator`; it is never published.
*Avoid*: "the package" (that is the generated artifact, below).

**Generated package** (`pubmed_proto`):
...
*Avoid*: "pubmed-proto" (that is the generator repo).
...
(`build.version`). Drops book/admin types, flattens list wrappers, coerces
booleans/timestamps, serializes rich-text to markdown.
